...
class BookingController extends Controller
{
    ...
    public function selectSeats(Request $request)
    {
        ...
        $booking_data = Booking::where('user_email', $request->session()->get('LoggedUserEmail'))
        ->where('route_id', $route->id)->get()->toArray();
        $seats_data = [];
        if(!empty($booking_data)){
            $seats_data = array_column($booking_data, 'selected_seats');
        }
        // echo "<pre>";
        // print_r($seats_data);die;
...
public function confirmBooking(Request $request)
    ...
    public function booking_view(Request $request, $id)
    {
        ...
        $seats = Seat::where('seat_type', $booking_data[0]['route']['seat_type'])
        // ->where('is_booked', false)
        ->get();
